...
export function webEdior(callback) {
    ...
    var labelList = function(){
        $eventBus.$emit('labelList', labelListdata)
    }
    ...
    var addSceneLabel = function(name,type,path){
      var modelNum;
      var objectNum;
      if (labelListdata.length > 0) { // 定义对象数组，便于后期的子对象的参数使用；
        modelNum = labelListdata[labelListdata.length - 1].id.split('B')[1];
        objectNum = parseInt(modelNum) + 1;
    } else {
        objectNum = 10; // 定义模型的初始数据，避免出现11 与 1 的问题
    }
        ...
        sprite.name = "labeldataB"+objectNum;;
        sprite.scale.set(1000,1000,1);
        // control.attach(sprite);
        labelGroup.add( sprite );
        labelListdata.push({
          id:"labeldataB"+objectNum,  // labelataB0
          name:name,  // 标签名称
          position:{x:0,y:0,z:0},
          type:type, // 标签类型
          path:path//标签路径
        ...
        labelList();    // 调用刷新监控场景数据的方法;


        var scenedata = JSON.parse( localStorage.getItem('sceneData'));   // 将标签数据保存到
        scenedata.scene3D.label.push({
          id:"labeldataB"+objectNum,  // labelataB0
          name:name,  // 标签名称
          position:{x:0,y:0,z:0},
          type:type, // 标签类型
    ...
    var updateSceneLabel = function(type,id,value){
       // 1.清理场景内部的对象labelGroup
      // 2.删除labelListdata中的对象
      // 3.清理localstorage 中的数据
      console.log(type);
      console.log(id);
      console.log(value);
        switch(type) {
          case "name":
         labelListdata.filter(item => {
              if( item.id == id){
                item.name == value;
              }
            });
            var scenedata = JSON.parse( localStorage.getItem('sceneData'));
            // console.log(scenedata);
            scenedata.scene3D.label.filter(item => {
              if( item.id == id){
                item.name = value;
              }
            })
            // console.log(scenedata.scene3D.label);
            localStorage.setItem('sceneData',JSON.stringify(scenedata));

            break;
          case "type":    // 切换标签类型的时候,一定会切换标签的路径信息
          // 当给的是切换标签的时候，调用一次修改type的方法，再调用一次修改path的方法
            labelListdata.filter(item => {
              if( item.id == id){
                item.type = value;
              }
            ...
            break;
          case "path":
            labelListdata = labelListdata.filter(item => {
              if( item.id == id){
                item.path = value;
              }
            });
            ...
            // 同时更新labelGroup的材质
            labelGroup.children.filter(child => {
              if(child.id == id){
    ...
    var deleteSceneLabel = function(id){
        // console.log(id);
      if(!id) return;
    ...
    //   console.log(labelListdata);
      labelList();
    var scenedata = JSON.parse( localStorage.getItem('sceneData'));   // 将标签数据保存到
    scenedata.scene3D.label = scenedata.scene3D.label.filter(item => {
      return item.id != id;
    })
